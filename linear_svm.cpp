...
int ass_class(float d[30], int size, float original_class, int maxclass) //to assign class based on minimum Mahalanobis distance
{
	int j;
	...
	for (j=1; j<=maxclass; j++)
	{
		...
		if(i!=j)
		{
			for(cn=0;cn<size;cn++)
			{
				sum[j]  = sum[j]+training[i].inner[j].weight[cn]*test.test_input[cn];
			}
			sum[j]  = sum[j]+training[i].inner[j].bias;
		//	printf("\n sum : %f    j : %d",sum[j],j);
		}
	}
...
{	
	...
	FILE *fp1=fopen("test_data2.txt", "w");
	FILE *fp2=fopen("training_data2.txt", "w");
...
//test data without repetitions
	rewind(fp);
	FILE *fp3=fopen("new_test_data2.txt", "w+");
...
//training data	
	...
	fp2=fopen("training_data2.txt", "r");
	int cls[30];
	int num_total=0;
	int q;
	int vprev=1;
...
for(i=1;i<=max_class;i++)    //max_class
	...
	FILE *fk=fopen("new_test_data2.txt","r");
	line=1;
	rewind(fk);
	...
	while(line<=newloop)
	{
		...
		value_miscls = ass_class(test.test_input,dim-1,var,max_class);
